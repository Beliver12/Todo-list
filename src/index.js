...
export function component() {
  ...
  function createTodoDivs2(e) {
    ...
    for (let i = 0; i < myProjects[values].storage.length; i++) {
      ...
      myProjects[values].storage[i].dueDate = format(
        new Date(myProjects[values].storage[i].dueDate),
        "MM/dd/yyyy",
      );
  ...
  function createTodoDivs(e) {
    ...
    if (
      value === "" ||
      myProjects[value2].name !==
        document.getElementById("todoContent").children[0].children[3]
          .childNodes[0].data
    ) {
      value2 = value;
    }
    ...
    for (let i = 0; i < myProjects[value2].storage.length; i++) {
      ...
      todoDivs.setAttribute("id", "todoDiv");
      todoContent.appendChild(todoDivs);
      myProjects[value2].storage[i].dueDate = format(
        new Date(myProjects[value2].storage[i].dueDate),
        "MM/dd/yyyy",
      );
      ...
      delBtn.addEventListener("click", (e) => {
        ...
        for (let i = 0; i < myProjects.length; ) {
          if (
            document.getElementById("displayProjectBox").children[i].children[0]
              .childNodes[0].data !==
            document.getElementById("todoContent").children[0].children[3]
              .childNodes[0].data
          ) {
            ...
            projectDivVal.push(
              document.getElementById("displayProjectBox").children[i]
                .children[1].attributes[0].value,
            );
            removeTodos(todoDivs);
            createTodoDivs();
            return false;
  ...
  function createProjectDivs() {
    ...
    for (let i = 0; i < myProjects.length; i++) {
      ...
      const { value } = document.getElementById("show");
      ...
      addProject.addEventListener("click", () => {
        ...
        const name = document.getElementById("projectBox").value;
      ...
      projectDel.addEventListener("click", (e) => {
        ...
        if (
          todoDivs === null ||
          document.getElementById("todoDiv").children.item(3).innerHTML ===
            projectDivText
        ) {
          clearContent(todoContent);
        }
      ...
      if (userData[i].name !== myProjects[i].name) {
        ...
        for (let j = 0; j < userData[i].storage.length; j++) {
          if (
            ...
            todoContent.appendChild(todoDivs);
            userData[i].storage[j].dueDate = format(
              new Date(userData[i].storage[j].dueDate),
              "MM/dd/yyyy",
            );
  ...
  function displayProjectTodos(e) {
    ...
    for (let i = 0; i < myProjects[value].storage.length; i++) {
      ...
      todoContent.setAttribute("value", i);
      todoContent.appendChild(todoDivs);
      myProjects[value].storage[i].dueDate = format(
        new Date(myProjects[value].storage[i].dueDate),
        "MM/dd/yyyy",
      );
  ...
  function editTodo(e) {
    ...
    for (let i = 0; i < myProjects.length; ) {
      if (
        ...
        document.getElementById("taskTitle").value =
          myProjects[i].storage[values].title;
        document.getElementById("description").value =
          myProjects[i].storage[values].description;
        document.getElementById("taskDate").value =
          myProjects[i].storage[values].dueDate;
        document.getElementById("select").value =
          myProjects[i].storage[values].project;
        return false;
      }
    }
  }
  ...
  function updateTodo() {
    ...
    const value = parseFloat(myValues);
    const title = document.getElementById("taskTitle").value;
    const description = document.getElementById("description").value;
    const dueDate = document.getElementById("taskDate").value;
    const project = e.options[e.selectedIndex].text;
    ...
    delBtn.addEventListener("click", (e) => {
      ...
      for (let i = 0; i < myProjects.length; ) {
        if (
          ...
          projectDivVal.push(
            document.getElementById("displayProjectBox").children[i].children[1]
              .attributes[0].value,
          );
          removeTodos(todoDivs);
          createTodoDivs();
  ...
  function displayTodos() {
    ...
    if (e.options[e.selectedIndex].text === "Choose --Project") {
  ...
  function activeProject() {
    ...
    for (let i = 0; i < childrens.length; i++) {
      if (document.getElementById("todoDiv") === null) {
        ...
        document.getElementById("todoDiv").children.item(3).innerHTML ===
        childrens[i].children.item(0).innerHTML
      ) {
  ...
  function setAttr(edit) {
    const element = document.getElementById("todoContent");
  ...
  function setName2(projectShowTodos) {
    const element = document.getElementById("displayProjectBox");
    const { children } = element;
    for (let i = 0; i < children.length; i++) {
